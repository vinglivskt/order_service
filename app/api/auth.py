...
@router.post(
    "/register/", response_model=SUserRead, status_code=status.HTTP_201_CREATED
)
...
async def register_user(
    ...
    if not any(ch.isdigit() for ch in payload.password) or not any(
        ch.isalpha() for ch in payload.password
    ):
        raise HTTPException(
